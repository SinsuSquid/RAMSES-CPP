...
static int get_nbor_of_coarse(const AmrGrid& grid, int icell, int idim, int side) {
    ...
    int ni = (i + di + nx) % nx, nj = (j + dj + ny) % ny, nk = (k + dk + nz) % nz;
    return nk * (nx * ny) + nj * nx + ni + 1;
}
...
void TreeUpdater::make_grid_fine(int ilevel) {
    ...
    if (ilevel == 1) {
        for (int i = 1; i <= grid_.ncoarse; ++i) if (grid_.flag1[i - 1] == 1 && grid_.son[i - 1] == 0) coarse_cells_to_refine.push_back(i);
    } else {
    ...
    for (int ic_coarse : coarse_cells_to_refine) {
        ...
        if (ilevel > 1) {
            ...
            int ign_ref[7] = {0};
            grid_.get_nbor_grids(ig_ref, ign_ref);
...
void TreeUpdater::remove_grid_fine(int ilevel) {
    ...
    while (ig > 0) {
        ...
        for (int ic = 1; ic <= n2d; ++ic) {
            int idc = grid_.ncoarse + (ic - 1) * grid_.ngridmax + ig - 1;
            if (grid_.flag1[idc] == 1 || grid_.son[idc] != 0) { has_refinement = true; break; }
        }
        int next_ig = grid_.next[ig - 1];
        if (!has_refinement && ilevel > config_.get_int("amr_params", "levelmin", 1)) {
            // (Placeholder for grid removal logic)
        }
        ig = next_ig;
    }
}
...
void TreeUpdater::flag_fine(int ilevel, real_t ed, real_t ep, real_t ev, real_t eb2, const std::vector<real_t>& evar, int nexp) {
    ...
    if (ilevel < lmin) {
        if (ilevel == 1) { 
            for (int i = 0; i < grid_.ncoarse; ++i) grid_.flag1[i] = 1; 
        }
        else {
            int ig = grid_.get_headl(myid, ilevel);
                ...
                for (int idim = 0; idim < NDIM; ++idim) {
                    ...
                    if (!evar.empty()) {
                        ...
                        for (int iv = 0; iv < (int)evar.size(); ++iv) {
                            ...
                            real_t qmc = grid_.uold(idc+1, idx_v)/dm;
                            real_t qlc = (il > 0) ? grid_.uold(il, idx_v)/dl : qmc;
                            real_t qrc = (ir > 0) ? grid_.uold(ir, idx_v)/dr : qmc;
                            real_t gr = 2.0 * std::max(std::abs(qrc - qmc) / (std::abs(qrc) + std::abs(qmc) + fd), std::abs(qmc - qlc) / (std::abs(qmc) + std::abs(qlc) + fd));
    ...
    // Perform expansion steps as specified (defaulting to 1 if 0)
