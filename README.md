...
* **Timestep Stabilization & Snapshot Alignment (Phase 51 ✅):** Resolved an off-by-one check in `flag_fine` that caused `compute_courant_step` to freeze the timestep, and shifted `dump_snapshot` logic out of the recursive `amr_step` to the main loop to ensure the final output is captured precisely at `t = tout` before simulation exit.
* **Coordinate-Based Neighbor Lookup (Phase 50 ✅):** Fixed major dimensionality traversal bugs in the AMR nested grid by replacing relative sibling chaining with robust absolute coordinate-based neighbor lookups.
