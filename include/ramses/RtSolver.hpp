...
public:
    ...
    virtual void apply_source_terms(int ilevel, real_t dt);
...
protected:
    virtual void load_hll_eigenvalues();
    virtual void rt_godfine1(const std::vector<int>& ind_grid, int ilevel, real_t dt, real_t dx);
    
    // M1 closure specific methods
    virtual void cmp_flux_tensors(int ncache, int iP0, int ilevel);
    virtual void cmp_eigenvals(int ncache, int iP0, int ilevel);
    virtual void inp_eigenvals(real_t ff, real_t omega, real_t& lmin, real_t& lmax);
    virtual void cmp_rt_faces(int ncache, int iP0, int ilevel, real_t dt, real_t dx);
